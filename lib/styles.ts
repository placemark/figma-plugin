...
export const getStyles = (reset: boolean) => {
  ...
  const STYLES: Record<GROUPS, () => Style> = {
    ...
    [GROUPS.OverlayPolygon]: once(() => {
      const fillStyle = createColorPaint("Polygon fill", [0.0, 0.9, 0.9], 0.5);
      const strokeStyle = createColorPaint("Polygon stroke", [0.0, 0.5, 0.5]);
      return {
        fillStyleId: fillStyle.id,
        strokeStyleId: strokeStyle.id,
        strokeWeight: 1,
    ...
    [GROUPS.OverlayLine]: once(() => {
      const strokeStyle = createColorPaint255("Line stroke", [0, 200, 200]);
      return {
        strokeStyleId: strokeStyle.id,
        strokeWeight: 4,
      };
    }),
    [GROUPS.OverlayPoint]: once(() => {
      const fillStyle = createColorPaint255("Overlay point", [50, 50, 50]);
      return {
        fillStyleId: fillStyle.id,
        radius: 4,
        strokeWeight: 0,
    ...
    [GROUPS.University]: once(() => {
      const fillStyle = createColorPaint255(
        "Education spaces",
        [254, 255, 230]
      );
      return {
